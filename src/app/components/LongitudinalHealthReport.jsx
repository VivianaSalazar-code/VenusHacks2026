...
function Sparkline({ data, trend }) {
  ...
  const color = trendConfig[trend]?.color || "#64748b";
  return (
...
function TrendBadge({ trend }) {
  const cfg = trendConfig[trend] || { color: "#64748b", bg: "#f1f5f9", label: trend };
  return (
    <span
      style={{
        display: "inline-flex",
...
export default function LongitudinalHealthReport() {
  ...
  useEffect(() => {
    ...
    font.rel = "stylesheet";
    font.href = "https://fonts.googleapis.com/css2?family=IBM+Plex+Mono:wght@400;600&family=IBM+Plex+Sans:wght@300;400;500;600;700&display=swap";
  ...
  const exportPDF = async () => {
    ...
    try {
      ...
      const W = 210, marginX = 18, contentW = W - marginX * 2;
      let y = 0;
      ...
      patientData.metrics.forEach((m, i) => {
        ...
        // Trend badge
        const cfg = trendConfig[m.trend] || { color: "#64748b" };
        const rgb = m.trend === "Concerning"
          ? [192, 57, 43]
          : m.trend === "Improving"
          ? [26, 122, 74]
          : [30, 58, 95];
      ...
      patientData.medications.forEach((med, i) => {
        doc.text(`• ${med}`, marginX + 5, y + 17 + i * 8);
      });
      ...
      doc.setFontSize(8);
      doc.setTextColor(60, 75, 95);
      patientData.careTeam.forEach((ct, i) => {
        doc.text(
          `• ${ct.role} — ${ct.name}`,
          cx2 + 5,
          y + 17 + i * 8
  ...
  return (
    <div style={{ fontFamily: "'IBM Plex Sans', sans-serif", background: "#f3f5f8", minHeight: "100vh", padding: "28px 0" }}>
      <div ref={reportRef} style={{ maxWidth: 840, margin: "0 auto", padding: "0 20px" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 22 }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <h1 style={{ margin: 0, fontSize: 26, fontWeight: 700, color: "#12264a", letterSpacing: "-0.02em" }}>
                Longitudinal Health Report
              </h1>
            </div>
            <p style={{ margin: "4px 0 0 36px", fontSize: 13, color: "#8a9ab5", fontWeight: 400 }}>
          ...
          <button
            ...
            style={{
              ...
              background: exporting ? "#d0604f" : "#e85d4a",
              color: "#fff",
              border: "none",
              borderRadius: 10,
              padding: "10px 20px",
        ...
        <div style={{
          ...
          padding: "20px 24px",
          marginBottom: 18,
          boxShadow: "0 1px 4px #12264a12",
          border: "1px solid #e4eaf4",
        }}>
          <div style={{ fontWeight: 700, fontSize: 16.5, color: "#12264a", marginBottom: 4 }}>
            {patientData.name}
          </div>
          <div style={{ fontSize: 12.5, color: "#7a8fac", marginBottom: 12, fontWeight: 400 }}>
            DOB {patientData.dob} · {patientData.pregnancy} · {patientData.location}
          </div>
          <p style={{ margin: 0, fontSize: 13.5, color: "#3c4f65", lineHeight: 1.7 }}>
            {patientData.summary}
          </p>
        </div>
        ...
        <div style={{
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ background: "#12264a" }}>
                {["Metric", "Current", "7-day", "30-day", "90-day", "Sparkline", "Trend"].map((h) => (
                  <th key={h} style={{
                    padding: "12px 14px", textAlign: "left", fontFamily: "'IBM Plex Mono', monospace",
                    fontWeight: 600, fontSize: 11, color: "#afc3dc", letterSpacing: "0.05em",
                    borderBottom: "none",
                  }}>{h}</th>
                ))}
              </tr>
            ...
            <tbody>
              {patientData.metrics.map((m, i) => (
                <tr key={m.name} style={{ background: i % 2 === 0 ? "#fafbfd" : "#fff", transition: "background 0.15s" }}>
                  <td style={{ padding: "13px 14px", fontWeight: 600, fontSize: 13.5, color: "#12264a", borderBottom: "1px solid #ecf0f8" }}>
                    {m.name}
                    {m.unit && <span style={{ color: "#8a9ab5", fontWeight: 400, fontSize: 11.5, marginLeft: 4 }}>({m.unit})</span>}
                  </td>
                  <td style={{ padding: "13px 14px", fontWeight: 600, fontSize: 13.5, color: "#1e3a5f", borderBottom: "1px solid #ecf0f8", fontFamily: "'IBM Plex Mono', monospace" }}>
                    {m.current}
                  </td>
                  {[m.day7, m.day30, m.day90].map((v, vi) => (
                    <td key={vi} style={{ padding: "13px 14px", fontSize: 12, color: "#5a6e88", borderBottom: "1px solid #ecf0f8", fontFamily: "'IBM Plex Mono', monospace" }}>
                      {v}
                    </td>
                  ))}
                  <td style={{ padding: "13px 14px", borderBottom: "1px solid #ecf0f8" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          ...
          <div style={{
            ...
            <div style={{ fontWeight: 700, fontSize: 14.5, color: "#12264a", marginBottom: 12 }}>Medications</div>
            <ul style={{ margin: 0, padding: 0, listStyle: "none" }}>
              {patientData.medications.map((med, i) => (
                <li key={i} style={{ fontSize: 13, color: "#3c4f65", padding: "4px 0", display: "flex", alignItems: "flex-start", gap: 8 }}>
                  <span style={{ color: "#8a9ab5", fontSize: 16, lineHeight: 1.2, flexShrink: 0 }}>·</span>
                  {med}
                </li>
              ))}
            </ul>
          ...
          <div style={{
            ...
            <div style={{ fontWeight: 700, fontSize: 14.5, color: "#12264a", marginBottom: 12 }}>Care Team</div>
            <ul style={{ margin: 0, padding: 0, listStyle: "none" }}>
              {patientData.careTeam.map((ct, i) => (
                <li key={i} style={{ fontSize: 13, color: "#3c4f65", padding: "4px 0", display: "flex", alignItems: "flex-start", gap: 8 }}>
                  ...
                  <span>
                    ...
                    {" — "}
                    <span style={{ color: ct.name === "pending" ? "#e85d4a" : "#3c4f65", fontStyle: ct.name === "pending" ? "italic" : "normal" }}>
                      {ct.name}
                    </span>
                  </span>
                </li>
        ...
        {/* Footer */}
        <div style={{ textAlign: "center", marginTop: 24, fontSize: 11, color: "#aab5c8", fontFamily: "'IBM Plex Mono', monospace" }}>
